...
float computeMOTP(const int matches, const double tot_dist, const float threshold, const bool world){
...
void computeMLPTMT(const clearMotMexRes_t& res, std::vector<tk::metrics::BoundingBox> gt, int max_frame_det, int& MT, int& PT, int& ML){
    MT = 0; //mostly tracked
...
void computeintIDTPFPFN(const std::vector<tk::metrics::BoundingBox>& gt, const std::vector<tk::metrics::BoundingBox>& det,  const float threshold, const bool world, int& IDTP,int& IDFP,int& IDFN, int& GT){
    ...
    std::vector<std::vector<tk::metrics::BoundingBox>> gt_per_trackID (gt_unique_gt_track_ID.size());
    int i=0;
    ...
    std::vector<std::vector<tk::metrics::BoundingBox>> det_per_trackID (det_unique_gt_track_ID.size());
    i=0;
...
tk::metrics::trackingMetrics_t computeTrackingMetrics(std::vector<tk::metrics::BoundingBox> gt, std::vector<tk::metrics::BoundingBox> det, const float threshold, const bool world, bool verbose){

    tk::metrics::trackingMetrics_t metrics;
